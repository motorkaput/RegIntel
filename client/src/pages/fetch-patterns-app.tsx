...
export default function FetchPatternsApp() {
  ...
  const exportToPDF = async () => {
    try {
      ...
      let pdfContent = `FETCH PATTERNS - COMPREHENSIVE ANALYSIS REPORT\n\n`;
      pdfContent += `Generated by: Beta User\n`;
      pdfContent += `Date: ${dateStr}\n`;
      pdfContent += `Time: ${timeStr}\n`;
      pdfContent += `Documents Analyzed: ${sessionAnalyses.length}\n\n`;
      
      pdfContent += `SESSION METRICS:\n`;
      pdfContent += `- Documents Processed: ${completedAnalyses.length}\n`;
      pdfContent += `- Unique Keywords: ${uniqueKeywords}\n`;
      pdfContent += `- High Confidence Documents: ${highConfidence}\n`;
      pdfContent += `- High Risk Documents: ${highRisk}\n\n`;
      
      // Document Analysis Section
      pdfContent += `DOCUMENT ANALYSES:\n\n`;
      completedAnalyses.forEach((analysis, index) => {
        pdfContent += `${index + 1}. ${analysis.originalName}\n`;
        pdfContent += `Classification: ${analysis.classification || 'N/A'}\n`;
        pdfContent += `Summary: ${analysis.summary || 'No summary available'}\n`;
        if (analysis.keywords && analysis.keywords.length > 0) {
          pdfContent += `Keywords: ${analysis.keywords.join(', ')}\n`;
        }
        if (analysis.sentiment) {
          pdfContent += `Sentiment: ${analysis.sentiment.label} (Score: ${analysis.sentiment.score})\n`;
        }
        pdfContent += `\n`;
      });
      ...
      if (questionHistory.length > 0) {
        pdfContent += `Q&A HISTORY:\n\n`;
        questionHistory.forEach((qa, index) => {
          pdfContent += `Q${index + 1}: ${qa.question}\n`;
          pdfContent += `A${index + 1}: ${qa.answer}\n`;
          pdfContent += `Confidence: ${(qa.confidence * 100).toFixed(1)}%\n\n`;
        });
      }
      
      // Context Analysis Section
      if (contextHistory.length > 0) {
        pdfContent += `CONTEXT ANALYSES:\n\n`;
        contextHistory.forEach((context, index) => {
          pdfContent += `Context ${index + 1}: ${context.context}\n`;
          pdfContent += `Mentions: ${context.mentions}\n`;
          pdfContent += `Summary: ${context.summary}\n`;
          pdfContent += `Sentiment - Positive: ${context.sentimentBreakdown.positive}%, `;
          pdfContent += `Negative: ${context.sentimentBreakdown.negative}%, `;
          pdfContent += `Neutral: ${context.sentimentBreakdown.neutral}%\n`;
          if (context.keyPhrases.length > 0) {
            pdfContent += `Key Phrases: ${context.keyPhrases.join(', ')}\n`;
          }
          pdfContent += `\n`;
        });
      }
      
      // Export as text file (PDF creation would require additional libraries)
      const blob = new Blob([pdfContent], { type: 'text/plain' });
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `FetchPatterns_Report_${currentDate.getFullYear()}-${String(currentDate.getMonth() + 1).padStart(2, '0')}-${String(currentDate.getDate()).padStart(2, '0')}_${String(currentDate.getHours()).padStart(2, '0')}-${String(currentDate.getMinutes()).padStart(2, '0')}.txt`;
      a.click();
      window.URL.revokeObjectURL(url);
      
      toast({
        title: "Report Downloaded",
        description: "Comprehensive analysis report has been downloaded successfully.",
      });
    } catch (error) {
      toast({
        title: "Export Failed",
